<x-pages.template>
    <x-slot name="title">オーナー新規登録 - Enjoyat</x-slot>
    ...
    <div class="lg:flex lg:justify-around mt-6 lg:mt-12 w-4/5 mx-auto">
        ...
        <div class="select-none lg:w-5/12 max-w-md mx-auto">
            ...
            <form method="POST" action="{{ route('owner.register') }}" id="register-form">
                ...
                <div class="mt-4">
                    <label>
                        <p>氏名<span class="text-red-500 text-xl md:text-2xl">*</span></p>
                        <input id="name" type="text" name="name" placeholder="山田太郎" value="{{ old('name') }}" class="w-full" required>
                        @error('name')
                            <span id="name_validation" class="text-red-500 text-sm font-semibold">※ {{ $message }}</span>
                        @enderror
                    </label>
                </div>
                ...
                <div class="lg:flex lg:justify-between mt-4">
                    ...
                    <a href="{{ route('owner.login') }}" class="block text-center mt-4 lg:mt-0 lg:w-6/12 px-8 py-4 text-sm font-bold rounded-lg border-gray-700 hover:border-gray-900 border-2 hover:bg-gray-100">
    ...
    <x-slot name="jsFile">
        <script src="{{ mix('js/owner/register.js') }}"></script>
    </x-slot>
</x-pages.template>
