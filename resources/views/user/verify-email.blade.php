<x-pages.template>
    <x-slot name="title">ユーザー会員登録受付画面 - {{ config('app.name') }}</x-slot>

    {{-- ユーザー会員登録受付画面 --}}
    <div class="mt-12 md:mt-20 mb-20 md:mb-40">
        <div class="w-4/5 lg:w-5/12 max-w-md mx-auto">
            <h1 class="font-bold"><span class="border-l-4 border-pink-400 pl-2"></span>ユーザー会員登録受付画面</h1>
            ...
            <div class="mt-8">
                <form method="POST" action="{{ route('user.verification.send') }}" id="resend-form">
                    @csrf
                    <div>
            ...
            <div class="mt-4">
                <form method="POST" action="{{ route('user.logout') }}" id="user-logout-form">
                    @csrf
                    <button type="submit" id="user-logout-button" class="block px-6 py-4 border-gray-700 hover:text-gray-100 hover:bg-gray-600 font-bold border-2 rounded-lg w-full lg:w-4/5 xl:w-9/12 lg:mx-auto">ログアウト</button>
                </form>
            </div>
        </div>
    ...
    <x-slot name="jsFile">
        <script src="{{ mix('js/user/verify-email.js') }}"></script>
    </x-slot>
</x-pages.template>
